...
describe Board do
  ...
  describe '#board_loc' do
    ...
    it 'returns the locations on the board' do
      locs = b.board_loc(number: 1, suite: 'H')
      expect(locs).to eq([1,50])
    end
  end

  describe '#valid_play?' do
    let(:board) { Board.build_horizontal }
    let(:cardI) { 1 }
    let(:card) { Deck.card(cardI) }
    let(:team) { 'R' }

    it 'returns true when valid' do
      expect(board.valid_play?(cardI, 0, 2)).to be true
    end


    it 'returns false when card doesnt match' do
      expect(board.valid_play?(cardI, 0, 0)).to be false
    end

    it 'returns false when location is full' do
      board.set_token(0,2, 'G')
      expect(board.valid_play?(cardI, 0, 2)).to be false
    end
  end
  ...
  describe '#check_new_sequence' do
    ...
    context 'horizontal: existing seq on left' do
      ...
      let(:board) { Board.build_horizontal.tap { |b| b.sequences = existing} }
