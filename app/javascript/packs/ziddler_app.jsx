...
// laydown is an array of arrays, compress it
// and remove empty arrays
const sanatizeLayingDown = (laydown) => {
    return laydown.filter((x) =>  x.length > 0 );
}

class Ziddler extends React.Component {
    ...
    loadWordList() {
        ...
        if (needLoad) {
            $.ajax({
                url: "https://ziddler-csw.s3-us-west-1.amazonaws.com/csw15.txt", //basePath + "csw15.txt",
                type: 'GET',
                success: (response) => {
                    if (typeof(Storage) !== "undefined") {
                        try {
    ...
    layingDown() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({laydown: layingDown}),
    ...
    onDragEnd(result) {
        ...
        if (source.droppableId == "playerHand" && destination.droppableId == "newLaydownWord") {
            //remove it from the handOrder
            const laydownCard = this.state.handOrder[source.index];
            const handOrder = remove(this.state.handOrder, source.index );
            layingDown = Array.from(this.state.layingDown || []); //TODO: deep copy...
            layingDown.push([laydownCard]);
            console.log(layingDown);
            this.setState({handOrder: handOrder, layingDown: layingDown}, this.layingDown);
        }

        if (source.droppableId.includes("laydown_") && destination.droppableId == "newLaydownWord") {
            const sourceWordI = parseInt(source.droppableId.split("laydown_")[1]);
            let layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
            const [cardI] = layingDown[sourceWordI].splice(source.index, 1);

            layingDown = sanatizeLayingDown(layingDown);
            layingDown.push([cardI]);
            this.setState({layingDownDiscard: null, layingDown: layingDown}, this.layingDown);
        }

        if (source.droppableId.includes("laydown_") && destination.droppableId == "playerHand") {
            const sourceWordI = parseInt(source.droppableId.split("laydown_")[1]);
            let layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
            const [cardI] = layingDown[sourceWordI].splice(source.index, 1);

            layingDown = sanatizeLayingDown(layingDown);
            let handState = addTo(this.state.handState, cardI, destination.index);
            this.setState({handState: handState, layingDown: layingDown}, this.layingDown);
        }

        if (source.droppableId == "laydownDiscard" && destination.droppableId == "newLaydownWord") {
            const cardI = this.state.layingDownDiscard;
            layingDown = Array.from(this.state.layingDown || []); //TODO: deep copy...
            layingDown.push([cardI]);
            this.setState({layingDownDiscard: null, layingDown: layingDown}, this.layingDown);
        }

        if (source.droppableId == "playerHand" && destination.droppableId.includes("laydown_")) {
            //remove it from the handOrder
        ...
        if (source.droppableId.includes("laydown_") && destination.droppableId.includes("laydown_")) {
            if (source.droppableId === destination.droppableId) {
                //intra word move
                const wordI = parseInt(destination.droppableId.split("laydown_")[1]);
                const layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
                layingDown[wordI] = reorder(layingDown[wordI], source.index, destination.index);

                this.setState({layingDown: layingDown}, this.layingDown);
            } else {
                //inter laydown word move
                const sourceWordI = parseInt(source.droppableId.split("laydown_")[1]);
                const destWordI = parseInt(destination.droppableId.split("laydown_")[1]);
                let layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
                const moveRes = move(layingDown[sourceWordI], layingDown[destWordI], source, destination);
                layingDown[sourceWordI] = moveRes[0];
                layingDown[destWordI] = moveRes[1];
                layingDown = sanatizeLayingDown(layingDown);
        ...
        if (source.droppableId == "laydownDiscard") {
            const cardI = this.state.layingDownDiscard;
            if (destination.droppableId == "playerHand") {
                this.setState({handOrder: addTo(this.state.handOrder, cardI, destination.index), layingDownDiscard: null}, this.layingDown);
            }
            if (destination.droppableId.includes("laydown_")) {
                const wordI = parseInt(destination.droppableId.split("laydown_")[1]);
                const layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
                layingDown[wordI] = addTo(layingDown[wordI], cardI, destination.index);

                this.setState({layingDown: layingDown, layingDownDiscard: null}, this.layingDown);
            }
        }

        if (destination.droppableId == "laydownDiscard") {
            if (source.droppableId == "playerHand") {
                const handOrder = Array.from(this.state.handOrder);
                const [cardI] = handOrder.splice(source.index, 1);
                this.setState({handOrder: handOrder, layingDownDiscard: cardI}, this.layingDown);
            }
            if (source.droppableId.includes("laydown_")) {
                const wordI = parseInt(source.droppableId.split("laydown_")[1]);
                const layingDown = Array.from(this.state.layingDown); //TODO: deep copy...
                const [cardI] = layingDown[wordI].splice(source.index, 1);

                this.setState({layingDown: layingDown, layingDownDiscard: cardI}, this.layingDown);
            }
        }
    }
    ...
    laydownTypeBoxChange(event) {
        ...
        //diff the current vs new and take an action based on the diffrence
...
document.addEventListener('DOMContentLoaded', () => {
    ReactDOM.render(
        <Ziddler gameId={window.gameId} gamePath={window.gamePath}/>,
